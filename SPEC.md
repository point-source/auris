...
| `AurisRadio` | Group-based single-select with a chamfered indicator (filled chamfered pip when selected) — Material's `Radio` is circular with no shape hook, so this is the geometric replacement. |
